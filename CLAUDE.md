...
1. **MCP (Model Context Protocol)**: Recommended. Preferred for full agent tool integration. Start via `zam mcp` or configure a harness via `zam agent connect <harness>` (supports `claude-code`, `claude-desktop`, `antigravity`, `codex`, `vscode`, `opencode`, `goose`, `copilot`, `hermes`, `zcode`).
