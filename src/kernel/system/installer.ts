...
export function installFastFlowLM(): InstallResult {
  ...
  try {
    ...
    execSync("winget install -e --id FastFlowLM --accept-source-agreements --accept-package-agreements", { stdio: "inherit" });
    return { success: true, message: "FastFlowLM installed successfully." };
  } catch (err) {
    return { success: false, message: `Failed to install FastFlowLM: ${(err as Error).message}` };
  }
}
...
export function installOllama(): InstallResult {
  ...
  if (process.platform === "darwin") {
    ...
    try {
      ...
      return { success: false, message: `Failed to install Ollama: ${(err as Error).message}` };
    }
  } else if (process.platform === "win32") {
    if (!hasCommand("winget")) {
      return { success: false, message: "winget was not found. Please install winget first." };
    }
