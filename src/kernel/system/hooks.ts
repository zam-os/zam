import { existsSync, mkdirSync, copyFileSync, appendFileSync, readFileSync, writeFileSync } from "node:fs";
 ...
 */
export function distributeGlobalSkills(): Array<{ name: string; path: string; success: boolean }> {
  ...
  try {
    ...
    results.push({ name: "Claude Code Global", path: join(claudeSkillsDir, "SKILL.md"), success: true });
  } catch (err) {
    results.push({ name: "Claude Code Global", path: claudeSkillsDir, success: false });
  }
  ...
  try {
    ...
    results.push({ name: "Gemini CLI Global", path: join(geminiSkillsDir, "SKILL.md"), success: true });
  } catch (err) {
    results.push({ name: "Gemini CLI Global", path: geminiSkillsDir, success: false });
  }
  ...
  try {
    ...
    results.push({ name: "Goose Global", path: join(gooseSkillsDir, "SKILL.md"), success: true });
  } catch (err) {
    results.push({ name: "Goose Global", path: gooseSkillsDir, success: false });
  }
 ...
 */
export function injectShellHooks(): Array<{ shell: string; file: string; success: boolean; alreadyHooked: boolean }> {
  const results: Array<{ shell: string; file: string; success: boolean; alreadyHooked: boolean }> = [];
  ...
  if (existsSync(zshrc)) {
    try {
      ...
      if (content.includes("zam monitor start")) {
        results.push({ shell: "zsh", file: zshrc, success: true, alreadyHooked: true });
      } else {
        appendFileSync(zshrc, hookLine);
        results.push({ shell: "zsh", file: zshrc, success: true, alreadyHooked: false });
      }
    } catch {
      results.push({ shell: "zsh", file: zshrc, success: false, alreadyHooked: false });
    }
  }
  ...
  if (existsSync(bashrc)) {
    try {
      ...
      if (content.includes("zam monitor start")) {
        results.push({ shell: "bash", file: bashrc, success: true, alreadyHooked: true });
      } else {
        appendFileSync(bashrc, hookLine);
        results.push({ shell: "bash", file: bashrc, success: true, alreadyHooked: false });
      }
    } catch {
      results.push({ shell: "bash", file: bashrc, success: false, alreadyHooked: false });
    }
  }
  ...
  for (const dir of pwshDirs) {
    ...
    try {
      ...
      if (content.includes("zam monitor start")) {
        results.push({ shell: "powershell", file: profileFile, success: true, alreadyHooked: true });
      } else {
        appendFileSync(profileFile, pwshHookLine);
        results.push({ shell: "powershell", file: profileFile, success: true, alreadyHooked: false });
      }
    } catch {
      results.push({ shell: "powershell", file: profileFile, success: false, alreadyHooked: false });
    }
  }
