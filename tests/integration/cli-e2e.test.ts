...
describe("cli E2E tests", () => {
  ...
  it(
    "can run setup to initialize database and setup identity via whoami",
    () => {
  ...
  it(
    "can get and set observer policy settings",
    () => {
      // Initialize DB
  ...
  it(
    "bridge get-observer-policy returns the parsed presets",
    () => {
      runCli(["setup", "--skip-claude-md", "--skip-agents-md"]);
      runCli(["whoami", "--set", "e2e-test-user"]);
  ...
  it(
    "can manage personal cards via bridge commands",
    () => {
      runCli(["setup", "--skip-claude-md", "--skip-agents-md"]);
      runCli(["whoami", "--set", "e2e-test-user"]);
      ...
      // 6. Verify the personal card disappears from the personal catalog.
      const listRes2 = JSON.parse(runCli(["bridge", "personal-card-list"]));
      expect(listRes2.cards).toHaveLength(0);
