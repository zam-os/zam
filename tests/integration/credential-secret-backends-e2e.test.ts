...
/**
 * Skipped on Windows — still, and honestly.
 *
 * `execFile("bw", …)` does not apply PATHEXT there, so the `bw.cmd` stub below
 * is never invoked and the vault reads "not installed". That is a real gap for
 * Windows learners too: the npm package `@bitwarden/cli` installs exactly such
 * a shim.
 *
 * `src/kernel/secrets/bw-executable.ts` was written to close it and does not,
 * yet. What is known after two CI rounds on windows-arm64:
 *
 * - the resolver is bundled into `dist/` and is the only thing that spawns
 *   `bw` (no `execFile("bw", …)` call sites remain);
 * - it resolves this exact shim shape correctly when unit-tested with
 *   `platform: "win32"`, including spawning the result;
 * - on the real runner the spawn still fails with ENOENT, i.e. the resolver
 *   returned its bare-name fallback. So PATH lookup or the candidate check
 *   behaves differently there in a way that is not reproducible off Windows.
 *
 * Diagnosing that needs instrumentation on a Windows runner, not another
 * guess. Until then this suite must not assert behaviour that never executes.
 * The resolver stays: it is tested, inert on POSIX, and does fix the
 * standalone `bw.exe` case, which plain `execFile` could not find either.
 *
 * Whatever the fix turns out to be, it must not be `shell: true` — ADR
 * 2026-07-30b Decision 11 moved the master password off the command line
 * precisely because other processes can read it there.
 */
