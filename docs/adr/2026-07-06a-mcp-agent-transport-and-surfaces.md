...
|---|---|---|
| **Antigravity CLI** (replaced Gemini CLI 2026-06-18) | Permission presets (`request-review` default, `proceed-in-sandbox`, `always-proceed`, `strict`); allowlist quirks inherited | Central `~/.gemini/config/mcp_config.json` shared with the IDE; per-server trust; first-use approval; even `strict` prompts only for *non-read* tools |
| **Antigravity IDE** | Terminal auto-execution allowlist demonstrably buggy (pipes/quotes re-prompt) | Per-project MCP tool allowlist; per-call Accept (`alwaysAllow` ignored as of 2026-03) — weakest MCP story, improving |
...
The pattern: MCP consent is **structured** (typed tool + JSON arguments → one
durable trust decision), shell consent is **textual** (every quoting variation is
a new string to match). Accepting an MCP server effectively means accepting its
functions — which is the consent granularity a learning tool needs.
...
|---|---|---|---|
| **T1: harness-owned chat** | Claude Code / Codex / Antigravity chat pane + ZAM MCP + optional panel | Developer doing real work (primary today) | One-time server trust; ~0 prompts |
| **T2: ZAM App embedded terminal** | Studio embeds a PTY terminal running the detected harness CLI | Learner who shouldn't juggle apps | ZAM provisions trusted MCP config before spawn; ~0 prompts |
...
| `zam_session_start` | start-session | — |
| `zam_session_end` | end-session (incl. `synthesize`) | — |
| `zam_get_reviews` | full due batch with resolved contexts (replaces per-card get-review *and* the `/tmp/zam-review.json` redirect) | read-only |
| `zam_submit_review` | rating **plus** session-step log, atomically (closes the `sessionId` gap) | — |
| `zam_review_action` | skip / edit / deprecate / delete / stop | destructive; keeps the in-protocol `confirm` gate |
| `zam_add_token` | add-token (token + card + prereqs + contexts) | — |
| `zam_find_tokens` | semantic find / relevant-tokens | read-only |
| `zam_suggest_foundations` | suggest-foundations | read-only |
| `zam_link_prereq` | prereq edge + optional block | — |
| `zam_monitor` | get-monitor + analyze-monitor | read-only |

Read-only tools get `readOnlyHint` (Antigravity `strict` and Codex per-tool modes
reward this); destructive actions keep ZAM's own `confirm` requirement so
**one-time server trust never silently authorizes deletion**. Tool descriptions
stay terse — the pedagogy lives in the skill, not in schemas.
...
### 4. `zam agent connect <harness>` — provision trust, don't document it
...
project `.mcp.json` (Claude Code), `~/.gemini/config/mcp_config.json`
(Antigravity IDE + CLI), `~/.codex/config.toml` (Codex, incl. per-tool
`approval_mode` for the destructive tools), opencode config. This powers the
Studio "Open Agent" button (ADR 2026-06-23 item 6) and is what later makes the
embedded terminal (T2) near-zero-prompt: ZAM provisions trust before spawning
the harness.
...
**Easier**
- A learning session in any MCP-capable harness needs one trust decision, not
  ~25 prompts; in Antigravity `strict` the read tools pass without prompting.
...
**Harder**
- Coarser consent is real: mitigated by annotations, ZAM-side `confirm` gates on
  destructive tools, and per-tool approval modes where hosts offer them.
...
1. [ ] **Extract transport-neutral handlers** from
...
2. [ ] **`zam mcp`** (`src/cli/commands/mcp.ts`): stdio server over
...
3. [ ] **Batch verbs on the bridge CLI too** (fallback parity): wire
   `--session` into `bridge submit` (rating + session step atomically); add a
   `bridge session-open` that returns stats + due summary + relevant tokens in
   one call.
4. [ ] **`zam agent connect <harness>`** in
...
5. [ ] **Slim the skill** (all three flavors + `AGENTS.md`): tool references,
...
8. [ ] **Re-verify the harness consent matrix** during items 2/4 — presets,
