...
1. **One token per article.** An umbrella "know the article" token
   schedules re-reading, not recall. Import must always yield multiple
   tokens.
...
3. **A `zam_okf_import` batch tool records a finished decomposition
   ...
   The tool validates — **at least two tokens** (a single-token import is
   rejected with an instructive error), unique names, acyclic
   ...
   edges, and source links in one transaction. Re-import is idempotent:
   tokens are matched by `source_link` + name and updated, not
   duplicated. The same operation is exposed as a `zam bridge` command.

4. **The panel jump goes through the conversation, not around it.** The
...
5. **Kernel boundary unchanged.** The tool composes existing kernel
   primitives (token/card creation, prerequisite linking) in the CLI
   layer; if atomicity requires a small kernel-side transaction helper,
   it is added without new dependencies and without any AI awareness.
...
- **Single umbrella token per article** — rejected by requirement: it
  schedules re-reading, not recall.
...
- Tests: tool contract (minimum-two rejection, cycle rejection,
  idempotent re-import, card creation for the importing user); panel
  fallback when `sendMessage` is rejected.
